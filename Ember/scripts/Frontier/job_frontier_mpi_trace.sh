#!/bin/bash
#SBATCH -A account
#SBATCH -J Ember_Halo3d-26_Frontier_TRACE
#SBATCH -o %x-%j.out
#SBATCH -t 15
#SBATCH -N 125
#SBATCH -c 7
...
for np in 125; do
    ...
    set -x
    srun -v -N $np -n $n -c 7 --gpus-per-task=1 --gpu-bind=closest --unbuffered -w ${nodelist} \
        --export=ALL,LD_PRELOAD=/lustre/orion/account/world-shared/user/network_test_scripts/sw_installs/llnl-wrap/custom_mpi_trace/mpi_trace_frontier_cpe22.12.so \
