...
parser.add_argument('--base', type=str, action='store', required=True, help="Path of the correct LAMMPS log file.")
...
def energy_breakdown(f_base, f_input):
...
# Check if both args.input and args.base are valid files
if not os.path.isfile(args.input):
    print("LAMMPS input file not found. Exiting.")
    exit(1)
elif not os.path.isfile(args.base):
    print("Base (correct) log file not found. Exiting.")
    exit(1)

if not args.quiet:
    print(f"Checking: {args.input} against {args.base} for correctness.")

check_result = check_file(f"{args.input}")
if not check_result:
    print("check_file failed, aborting check.")
    exit(1)
if not energy_breakdown(f"{args.base}", f"{args.input}"):
