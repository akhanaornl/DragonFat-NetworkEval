...
parser.add_argument('--key', type=str, default=f'{os.environ["LSB_DJOB_HOSTFILE"]}', action='store', help="File name of a file containing the nodes in the job.")
parser.add_argument('-n', type=int, required=True, action='store', help="Number of nodes to generate binding for.")
parser.add_argument('-o', '--output', type=str, default='input.erf', action='store', help="Name of output file to store the explicit resource binding.")
...
def read_key_file():
    """ Reads in the nodelist and returns a dictionary of node_name: rack_id """
    ret = {}
...
def get_largest_group(groups):
    ...
    for node in groups.keys():
        if not groups[node] in group_counts.keys():
            group_counts[groups[node]] = 1
        else:
            group_counts[groups[node]] += 1
        if group_counts[groups[node]] > max_count:
            max_count = group_counts[groups[node]]
            max_group = groups[node]
